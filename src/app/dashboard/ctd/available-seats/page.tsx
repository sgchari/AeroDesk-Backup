...
import { Plane, Users, Calendar, Search, Filter, Info, Share2, ArrowRight, ShieldCheck, Zap } from "lucide-react";
        ...
        <Card className="bg-card flex flex-col group hover:border-accent/40 transition-all duration-300 w-full overflow-hidden h-full">
            <CardHeader className="pb-3 pt-6 px-6 relative">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <Badge variant="outline" className="bg-white/5 border-white/10 text-[8px] uppercase tracking-widest font-black">
                        {leg.aircraftType || 'Jet'}
                    </Badge>
                </div>
            ...
            <CardContent className="flex-grow space-y-6 px-6">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1 text-right">
                        <p className="text-[8px] uppercase font-black text-muted-foreground tracking-[0.2em]">Available Seats</p>
                        <div className="flex items-center justify-end text-sm text-green-500 font-black">
                            <Users className="h-3.5 w-3.5 mr-2 text-accent/60" />
                            <span>{leg.availableSeats}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-accent/5 border border-accent/10 flex items-center justify-between">
                    <div className="space-y-0.5">
                        <p className="text-[8px] uppercase font-black text-accent tracking-widest">Rate Per Seat</p>
                        <p className="text-lg font-black text-white">₹ {leg.pricePerSeat?.toLocaleString() || '---'}</p>
                    </div>
                    <div className="text-right space-y-0.5">
                        <p className="text-[8px] uppercase font-black text-muted-foreground tracking-widest">Min. Req</p>
                        <p className="text-sm font-bold text-foreground">{leg.minSeatsPerRequest || 1} PAX</p>
                    </div>
                </div>
                
                <div className="flex items-start gap-3 p-3 bg-muted/20 rounded-lg border border-white/5 italic text-[10px] text-muted-foreground leading-relaxed">
                    <Info className="h-3.5 w-3.5 shrink-0 text-accent/40 mt-0.5" />
                    Corporate seat allocations are governed by your organizational travel policy. Final confirmation subject to NSOP manifest clearance.
                </div>
            </CardContent>
...
export default function AvailableSeatsPage() {
  ...
  return (
    <div className="space-y-10">
      <PageHeader title="Corporate Seat Inventory" description={`Approved positioning flights available for ${user?.company} personnel.`} />
        ...
        <>
            {filteredLegs && filteredLegs.length > 0 ? (
                ...
                <div className="text-center py-24 border-2 border-dashed rounded-xl bg-card/20 border-white/5 max-w-3xl mx-auto">
                    ...
                    <h3 className="text-xl font-bold text-white uppercase tracking-widest">No matching sectors</h3>
                    <p className="mt-2 text-sm text-muted-foreground max-w-sm mx-auto italic">
                        Standing by for operator repositioning updates.
                    </p>
                </div>
            )}
        </>
