...
export function BookingWidget() {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto bg-[#48D1CC]/80 backdrop-blur-md rounded-xl shadow-lg p-4 sm:p-6">
        <Tabs defaultValue="jet" className="w-full">
            <TabsList className="flex justify-center bg-transparent p-0 rounded-lg max-w-md mx-auto">
                ...
                <TabsTrigger value="helicopter" className="text-cyan-900/80 data-[state=active]:text-accent-foreground data-[state=active]:bg-accent data-[state=active]:shadow-lg p-3 rounded-md flex items-center justify-center gap-2 text-sm sm:text-base">
                    <Helicopter /> HELICOPTER
                </TabsTrigger>
                 <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger asChild>
                             <TabsTrigger value="seats" className="text-cyan-900/80 data-[state=active]:text-accent-foreground data-[state=active]:bg-accent data-[state=active]:shadow-lg p-3 rounded-md flex items-center justify-center gap-2 text-sm sm:text-base">
                ...
                <div className="space-y-6">
                    <RadioGroup value={tripType} onValueChange={handleTripTypeChange} className="flex items-center justify-center gap-4 sm:gap-6">
                        <div className="flex items-center space-x-2">
                            <RadioGroupItem value="oneway" id="oneway" className="border-cyan-800 text-cyan-800" />
                            <Label htmlFor="oneway" className="text-cyan-900 text-sm sm:text-base">Oneway</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                            <RadioGroupItem value="round" id="round" className="border-cyan-800 text-cyan-800" />
                            <Label htmlFor="round" className="text-cyan-900 text-sm sm:text-base">Round</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                            <RadioGroupItem value="multicity" id="multicity" className="border-cyan-800 text-cyan-800" />
                            <Label htmlFor="multicity" className="text-cyan-900 text-sm sm:text-base">Multicity</Label>
                        </div>
                    </RadioGroup>

                    <div className="flex flex-col bg-background/80 rounded-lg">
                        {tripType === 'multicity' ? (
                            <>
                                <div className="flex-1 flex flex-col">
                                    {legs.map((leg, index) => (
                                        <div key={index} className={cn("flex flex-col sm:flex-row items-stretch", index > 0 && "border-t border-cyan-200/50")}>
                                            <div className='flex-1'>
                                                <AutocompleteInput placeholder={`Leg ${index + 1} Origin`} value={leg.origin} onChange={(v) => updateLeg(index, 'origin', v)} />
                                            </div>
                                            <div className="h-px w-full sm:h-auto sm:w-px bg-cyan-200/50 self-stretch"></div>
                                            <div className='flex-1'>
                                ...
                                <div className="flex flex-col lg:flex-row items-stretch border-t border-cyan-200/50">
                                 ...
                                 <div className="p-2 bg-background/20 border-t border-cyan-200/50">
                            ...
                            <div className="flex flex-col lg:flex-row items-stretch">
                                ...
                                <div className="h-px w-full lg:h-auto lg:w-px bg-cyan-200/50 self-stretch"></div>
                                <div className="flex-1">
            ...
            <TabsContent value="helicopter"><p className="text-center p-12 text-lg text-cyan-900">Helicopter booking functionality coming soon.</p></TabsContent>
            <TabsContent value="seats"><p className="text-center p-12 text-lg text-cyan-900">Seat reservation functionality coming soon.</p></TabsContent>
        </Tabs>
    </div>
  );
}
