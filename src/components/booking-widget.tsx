    ...
    <div className="bg-black/20 p-4 rounded-lg shadow-2xl border border-white/10 backdrop-blur-sm">
        <TabsContent value="jet" forceMount={true} hidden={activeTab !== 'jet'} className="mt-0">
            <div className="space-y-4">
                ...
                {tripType === 'oneway' && (
                     <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-px rounded-md overflow-hidden bg-white/5 backdrop-blur-sm border border-white/10">
                ...
                {tripType === 'round' && (
                    <div className="space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-px rounded-md overflow-hidden bg-white/5 backdrop-blur-sm border border-white/10">
                            <AutocompleteInput placeholder="Origin" value={origin} onChange={setOrigin} />
                            <AutocompleteInput placeholder="Destination" value={destination} onChange={setDestination} />
                            <Input type="number" placeholder="Passengers" value={passengers} onChange={e => setPassengers(e.target.value)} className="bg-transparent text-white placeholder:text-white/70 border-0 rounded-none focus-visible:ring-0 focus-visible:ring-offset-0 text-center w-full h-full py-3" />
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-px rounded-md overflow-hidden bg-white/5 backdrop-blur-sm border border-white/10">
                ...
                {tripType === 'multicity' && (
                    <div className="space-y-4">
                        <div className="space-y-2">
                            {legs.map((leg) => (
                                <div key={leg.id} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr_1fr_auto] gap-px items-stretch rounded-md overflow-hidden bg-white/5 backdrop-blur-sm border border-white/10">
                        ...
                        <div className="grid grid-cols-1">
                             <Input 
                                ...
                                className="bg-white/5 backdrop-blur-sm border border-white/10 text-white placeholder:text-white/70 rounded-md h-full text-center"
                            />
                        </div>
                    </div>
                )}
