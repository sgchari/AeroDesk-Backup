 ...
 * Handles precision projection mapping for the India landmass using geographic bounds.
 */
...
// Adjusted to ensure the landmass fits perfectly within a 1000x1000 viewbox
const MAP_BOUNDS = {
    minLng: 68.0, 
    maxLng: 98.0, 
    minLat: 6.0,  
 ...
 * Uses a linear mapping that approximates geographic projection for visual consistency.
 */
export function project(lat: number, lng: number): { x: number; y: number } {
    // Horizontal mapping: (lng - min) / (max - min) * 1000
    const x = ((lng - MAP_BOUNDS.minLng) / (MAP_BOUNDS.maxLng - MAP_BOUNDS.minLng)) * 1000;
    
    // Vertical mapping: 1000 - ((lat - min) / (max - min) * 1000)
    // Note: y is inverted in SVG coordinate systems
...
export const hubGeographics: Record<string, GeoCoord & { type: 'backbone' | 'operator', label: string, operators: number, partners: number }> = {
    ...
    'Chennai': { lat: 13.0827, lng: 80.2707, type: 'backbone', label: 'Chennai Hub', operators: 2, partners: 3 },
...
/**
 * High-fidelity India boundary SVG path.
 * This is a high-resolution silhouette calibrated for the 1000x1000 viewbox.
 */
